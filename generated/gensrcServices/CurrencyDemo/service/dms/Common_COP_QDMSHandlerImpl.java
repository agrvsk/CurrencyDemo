...
/**
 * This handles the CRUD operations for implementing the Common_COP_QDMS
 * interface.
 *  
 * @author AUTOGENERATED
 * @see Common_COP_QDMS
 */
public class Common_COP_QDMSHandlerImpl extends VersataDMSHandlerBase implements Common_COP_QDMS
{
	private static final VersataLogger LOGGER = VersataLogger.getLogger(
		Common_COP_QDMSHandlerImpl.class);

	private static final Common_COP_QMarshaller _marshaller = new Common_COP_QMarshaller();
	...
	public Common_COP_QDMSHandlerImpl(String packageName, Session vsSession) {
	...
	public DataGraph getCommon_COP_Q(SearchRequest csr) throws VSORBException, ServerSecurityException {
	...
	// the code and comments does not handle exclusion list.
	// TODO: ensure that exclusion list is also handled.
	public DataGraph getCommon_COP_Q(String[] columnNames, Object[] columnValues) throws VSORBException, ServerSecurityException {
		...
		DataObject root = dataGraph.createRootObject(rootObject.getType().getURI(),"Common_COP_Q");
		...
		if(noOfFilters >= 1){
			WhereClause whereClause = new WhereClause(type.getProperty(columnNames[0]), type.getProperty(columnNames[0]), null, columnValues[0], WhereClause.CONDITION_EQUALS, "Common_COP_Q");
			rootFilter = new FilterImpl(whereClause);
		}
		for(int i=1;i<columnNames.length;i++){
			WhereClause whereClause = new WhereClause(type.getProperty(columnNames[i]), type.getProperty(columnNames[i]), null, columnValues[i], WhereClause.CONDITION_EQUALS, "Common_COP_Q");
			Filter tempFilter = new FilterImpl(whereClause);
			rootFilter = new FilterImpl(tempFilter,rootFilter,1,"Common_COP_Q");
		}
		SearchRequest csr = SearchRequestFactory.createFirstPageSearchRequest("Common_COP_Q", rootFilter, SearchRequest.NO_ROW_LIMIT, getDataGraph().getRootObject().getType().getProperty("common_COP_Q"), null, null);
	...
	public DataGraph updateCommon_COP_Q(SaveRequest saveRequest) throws VSORBException, ServerSecurityException {
