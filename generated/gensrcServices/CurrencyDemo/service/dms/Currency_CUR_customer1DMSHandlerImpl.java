...
/**
 * This handles the CRUD operations for implementing the Currency_CUR_customer1DMS
 * interface.
 *  
 * @author AUTOGENERATED
 * @see Currency_CUR_customer1DMS
 */
public class Currency_CUR_customer1DMSHandlerImpl extends VersataDMSHandlerBase implements Currency_CUR_customer1DMS
{
	private static final VersataLogger LOGGER = VersataLogger.getLogger(
		Currency_CUR_customer1DMSHandlerImpl.class);

	private static final Currency_CUR_customer1Marshaller _marshaller = new Currency_CUR_customer1Marshaller();
	...
	public Currency_CUR_customer1DMSHandlerImpl(String packageName, Session vsSession) {
	...
	public DataGraph getCurrency_CUR_customer1(SearchRequest csr) throws VSORBException, ServerSecurityException {
	...
	// the code and comments does not handle exclusion list.
	// TODO: ensure that exclusion list is also handled.
	public DataGraph getCurrency_CUR_customer1(String[] columnNames, Object[] columnValues) throws VSORBException, ServerSecurityException {
		...
		DataObject root = dataGraph.createRootObject(rootObject.getType().getURI(),"Currency_CUR_customer1");
		...
		if(noOfFilters >= 1){
			WhereClause whereClause = new WhereClause(type.getProperty(columnNames[0]), type.getProperty(columnNames[0]), null, columnValues[0], WhereClause.CONDITION_EQUALS, "Currency_CUR_customer1");
			rootFilter = new FilterImpl(whereClause);
		}
		for(int i=1;i<columnNames.length;i++){
			WhereClause whereClause = new WhereClause(type.getProperty(columnNames[i]), type.getProperty(columnNames[i]), null, columnValues[i], WhereClause.CONDITION_EQUALS, "Currency_CUR_customer1");
			Filter tempFilter = new FilterImpl(whereClause);
			rootFilter = new FilterImpl(tempFilter,rootFilter,1,"Currency_CUR_customer1");
		}
		SearchRequest csr = SearchRequestFactory.createFirstPageSearchRequest("Currency_CUR_customer1", rootFilter, SearchRequest.NO_ROW_LIMIT, getDataGraph().getRootObject().getType().getProperty("currency_CUR_customer1"), null, null);
	...
	public DataGraph updateCurrency_CUR_customer1(SaveRequest saveRequest) throws VSORBException, ServerSecurityException {
