...
*  
*/
 public abstract class OBJBaseImpl extends DataObject {
    
    public static VersataLogger logger = VersataLogger.getLogger(OBJImpl.class);
    
    protected OBJBaseImpl() {
			addListeners();
	...
	private static String modelName = "OBJ";
	...
	static {	
		...
		vsQueryDefinition= new VSQueryDefinition( "OBJ" );
		...
		VSMetaColumn c = null;
		VSMetaTable tableForSubObjRetQry = new VSMetaTable("OBJ");
		
		//{{META_QUERY_COLUMN_CTOR
		
		
		
			c = new VSMetaColumn("OBJ_OBJNR", DataConst.VARCHAR);
			...
			c = new VSMetaColumn("OBJ_BOSOK1_OMR", DataConst.VARCHAR);
			c.setColumnClassification(com.versata.tl.common.meta.VSMetaColumn.LOCAL);
			c.setSize(6);
			c.setAlterability(true);
			c.setNullability(VSMetaColumn.columnNullable);
			c.setCaption("BosÃ?Â¶k1omrÃ?Â¥de");
			tableForSubObjRetQry.addColumn( c );
			
    	
			c = new VSMetaColumn("OBJ_BOSOK2_OMR", DataConst.VARCHAR);
			c.setColumnClassification(com.versata.tl.common.meta.VSMetaColumn.LOCAL);
			c.setSize(6);
			c.setAlterability(true);
			c.setNullability(VSMetaColumn.columnNullable);
			c.setCaption("BosÃ?Â¶k2omrÃ?Â¥de");
			tableForSubObjRetQry.addColumn( c );
			
    			
		VSMetaTable t = getTableForObjRetQuery(tableForSubObjRetQry);
		t.addUniqueKeyColumn( "OBJ Unique Key", "OBJ_OBJNR" );
		  tableForSubObjRetQry.addUniqueKeyColumn( "OBJ Unique Key", "OBJ_OBJNR" );
...
// end  support for key generation
		...
		if(!"VLSJSFWebConsole".equals("CurrencyDemo"))
			qTemp = (VSQueryDefinition)getMetaQuery( "OBJ","CurrencyDemo" );			
							
		if ( qTemp == null ) {			
			vsQueryDefinition.setPartialSQLForDataModel("SELECT OBJ.OBJ_OBJNR AS OBJ_OBJNR, OBJ.OBJ_BOSOK1_OMR AS OBJ_BOSOK1_OMR, OBJ.OBJ_BOSOK2_OMR AS OBJ_BOSOK2_OMR FROM <dbschema>.OBJ OBJ", storageOption, inheritanceType, repositoryName, modelName);
	...
	private static VSMetaTable getTableForObjRetQuery(VSMetaTable t) {
		...
		VSMetaTable newMetaTable = new VSMetaTable("OBJ");
	...
	public String getPackageName() {
	   ...
	   //protected void handlers
		...
		protected void OBJ_afterCommit(Session session) {
			
		}

		protected void OBJ_afterDelete(DataObject obj) {

		}

		protected void OBJ_afterInsert(DataObject obj) {

		}

		protected void OBJ_afterRollback(Session session) {

		}

		protected void OBJ_afterUpdate(DataObject obj) {

		}

		protected void OBJ_beforeCommit(Session session, Response resp) {

		}

		protected void OBJ_beforeDelete(DataObject obj, Response resp) {

		}

		protected void OBJ_beforeInsert(DataObject obj, Response resp) {

		}

		protected void OBJ_beforeRollback(Session session) {

		}

		protected void OBJ_beforeUpdate(DataObject obj, Response resp) {

		}

		protected void OBJ_beginTrans(Session session) {
	   ...
	   protected void splitLogic(List listOfBOs, List childAttributesIhaveToHandle, List attributesThatWillBeHandledByMyChild) {		    
			if (storageOption == com.versata.tl.vls.core.StorageOptions.STORE_ALONE)        {
			    ...
			    OBJImpl implInstance = new OBJImpl();
...
/**	  
* <br>
* Constructor for the class OBJBaseImpl.
...
*/
public OBJBaseImpl(Session session, boolean makeDefaults) {
	super(session, OBJBaseImpl.getMetaQuery(), makeDefaults);
    ...
    public OBJBaseImpl(Session session, VSMetaQuery metaQuery, boolean makeDefaults) {
...
*/
	...
	com.versata.tl.vls.admin.AppObjectImpl appObject = com.versata.tl.vls.admin.AppObjectImpl.getBusinessObject( repositoryName, "OBJ", ServerEnvironment.getServer().getInternalSession());
	...
	initialized = true;
 	if (logger.isDebugEnabled())
 		logger.debug("OBJ cache size: " + cacheSize + " expiration " + expires);
}
...
throws ServerException
{
	...
	if ( aSession.getSecurityCheck() ) {
		try {
		if (!aSession.getMyPrivilegeToObjectName(DataConst.AppObjectPrivilegeImpl_READ, "OBJ", DataConst.AppObjectTypeCodeImpl_BUSINESS_OBJECT)) {
		throw new ServerException(ErrorMsgCode.VSMSG_SecurityNoReadAccess,  new Object[] {"business", "OBJ"});
		}
		}
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e = aSession.getTransactionInfo().getObjects(OBJBaseImpl.getSubObjectRetrievalMetaQuery(), xdac, searchReq);

		Vector dObjs = new Vector();
		while(e.hasMoreElements()) {
			OBJImpl implObj = (OBJImpl) e.nextElement();
			if(implObj == null) {
				continue;
			}
		...
		ResultSet rs = new VSORBResultSetImpl( OBJBaseImpl.getSubObjectRetrievalMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
			OBJImpl bo = (OBJImpl) row.getComponent();
			if(bo == null) {
				continue;
			}
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e= aSession.getTransactionInfo().getObjects(OBJBaseImpl.getMetaQuery(), xdac, searchReq);
		Vector boList = new Vector();
		while ( e.hasMoreElements()) {
			OBJImpl bo = (OBJImpl) e.nextElement();
		...
		if (cacheSize!=0)
		{
			CacheAgent.getCacheAgent().setObjects( OBJBaseImpl.getMetaQuery(), boList, expires );
		}
		return boList.elements();		
	} else {
		Vector boList = new Vector();
		Connection con = aSession.getConnection( xdac, true );
		ResultSet rs = new VSORBResultSetImpl( OBJBaseImpl.getMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
...
public static void raiseBeforeQueryEvent( SearchRequest searchReq, Session aSession )	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "OBJ.BeforeQuery") : null;
        try {
            beforeQuery( "OBJ", searchReq, null , aSession );
        }
	finally {
	    if (  tr != null ) tr.end();
        }
...
public static void raiseAfterQueryEvent( ResultSet rs)	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "OBJ.AfterQuery") : null;
        try {
	    afterQuery( rs );
        }
	finally {
	...
	throws ServerException
{
	...
	if(useSubObjSQL){
		OBJImpl implObj = (OBJImpl) getObjectByKeyUsingSubObjQueryInternal(key, aSession);
		if(implObj == null) {
			return null;
		}
		String tiAttrColData = implObj.get_VSTypeIndicatorAttrColumnData();
		...
		OBJImpl implObj = (OBJImpl) getObjectByKeyInternal(key, aSession, cacheOnly);
		if(implObj == null) {
			return null;
		}
	...
	throws ServerException
{
	...
	if ( cacheSize != 0 )    {
		anObject = CacheAgent.getCacheAgent().getObject( OBJBaseImpl.getMetaQuery(),key.parameterList, aSession);
		if ( logger.isDebugEnabled() ) {
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(OBJBaseImpl.getMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(OBJBaseImpl.getMetaQuery(),key);
	}
	
	// if we have a cache and got here, we had a cache miss so add this object to the global cache
	if ( cacheSize != 0 )
		 CacheAgent.getCacheAgent().addObject( OBJBaseImpl.getMetaQuery(),anObject, expires );
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(OBJBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(OBJBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	}
...
protected void insert_checkPrimaryKeyChanges() throws ServerException
{
    ...
    int pkChangeCount = 0;
    if (isChanged("OBJ_OBJNR")) {
        pkChangeCount++;
        }       
    if (pkChangeCount > 0) {
        getSession().getTransactionInfo().save(this, "OBJ");
    }
}
...
public void invalidateNonTransactionCaches() 
{
	...
	/**	  
	* <br>
	* method to get the OBJ_OBJNR attribute for the OBJ
	* @return String : the  value of the attribute OBJ_OBJNR as String.
	*/
	public String	getOBJ_OBJNR() 
	{
		return getData("OBJ_OBJNR").getString();
	}
	/**	  
	* <br>
	* method to set the OBJ_OBJNR attribute for the OBJ
	* @param value String : value of the attribute OBJ_OBJNR as String.
	*/
	public void	setOBJ_OBJNR(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OBJ_OBJNR");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OBJ_OBJNR attribute for the OBJ
	* @return String : the  value of the old attribute OBJ_OBJNR as String.
	*/
	public String	getOldOBJ_OBJNR()
	{
		return getData("OBJ_OBJNR").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the OBJ_BOSOK1_OMR attribute for the OBJ
	* @return String : the  value of the attribute OBJ_BOSOK1_OMR as String.
	*/
	public String	getOBJ_BOSOK1_OMR() 
	{
		return getData("OBJ_BOSOK1_OMR").getString();
	}
	/**	  
	* <br>
	* method to set the OBJ_BOSOK1_OMR attribute for the OBJ
	* @param value String : value of the attribute OBJ_BOSOK1_OMR as String.
	*/
	public void	setOBJ_BOSOK1_OMR(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OBJ_BOSOK1_OMR");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OBJ_BOSOK1_OMR attribute for the OBJ
	* @return String : the  value of the old attribute OBJ_BOSOK1_OMR as String.
	*/
	public String	getOldOBJ_BOSOK1_OMR()
	{
		return getData("OBJ_BOSOK1_OMR").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the OBJ_BOSOK2_OMR attribute for the OBJ
	* @return String : the  value of the attribute OBJ_BOSOK2_OMR as String.
	*/
	public String	getOBJ_BOSOK2_OMR() 
	{
		return getData("OBJ_BOSOK2_OMR").getString();
	}
	/**	  
	* <br>
	* method to set the OBJ_BOSOK2_OMR attribute for the OBJ
	* @param value String : value of the attribute OBJ_BOSOK2_OMR as String.
	*/
	public void	setOBJ_BOSOK2_OMR(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OBJ_BOSOK2_OMR");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OBJ_BOSOK2_OMR attribute for the OBJ
	* @return String : the  value of the old attribute OBJ_BOSOK2_OMR as String.
	*/
	public String	getOldOBJ_BOSOK2_OMR()
	{
		return getData("OBJ_BOSOK2_OMR").getPreviousString();
	}
...
throws ServerException
	...
	if ( aSession.isTransactionInProgress())
		count = aSession.getTransactionInfo().getObjectsCount(OBJBaseImpl.getMetaQuery(), xdac, searchReq);
	else
	{
		con = aSession.getConnection( xdac, true );
  	  	count = xdac.getRowCount( OBJBaseImpl.getMetaQuery(), searchReq, con );
	}
...
{ 
	...
	try {
		VSMetaTable table = OBJBaseImpl.getMetaQuery().getChildMostTable();
		if ( table != null )
...
{ 
	...
	Properties props = ((VSORBSessionImpl)aSession).getMyDataServiceLoginForObject("OBJ");
