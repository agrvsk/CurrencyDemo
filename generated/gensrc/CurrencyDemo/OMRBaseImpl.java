...
*  
*/
 public abstract class OMRBaseImpl extends DataObject {
    
    public static VersataLogger logger = VersataLogger.getLogger(OMRImpl.class);
    
    protected OMRBaseImpl() {
			addListeners();
	...
	private static String modelName = "OMR";
	...
	static {	
			...
			logger.debug("Initializing OMR");
		
		vsQueryDefinition= new VSQueryDefinition( "OMR" );
		...
		VSMetaColumn c = null;
		VSMetaTable tableForSubObjRetQry = new VSMetaTable("OMR");
		
		//{{META_QUERY_COLUMN_CTOR
		
		
		
			c = new VSMetaColumn("OMR_OMR", DataConst.VARCHAR);
			...
			c = new VSMetaColumn("OMR_BESK", DataConst.VARCHAR);
			...
			c = new VSMetaColumn("OMR_INGIOMR", DataConst.VARCHAR);
			c.setColumnClassification(com.versata.tl.common.meta.VSMetaColumn.LOCAL);
			c.setSize(6);
			c.setAlterability(true);
			c.setNullability(VSMetaColumn.columnNullable);
		...
		t.addUniqueKeyColumn( "OMR Unique Key", "OMR_OMR" );
		  tableForSubObjRetQry.addUniqueKeyColumn( "OMR Unique Key", "OMR_OMR" );
...
// end  support for key generation
		...
		if(!"VLSJSFWebConsole".equals("CurrencyDemo"))
			qTemp = (VSQueryDefinition)getMetaQuery( "OMR","CurrencyDemo" );			
							
		if ( qTemp == null ) {			
			vsQueryDefinition.setPartialSQLForDataModel("SELECT OMR.OMR_OMR AS OMR_OMR, OMR.OMR_BESK AS OMR_BESK, OMR.OMR_INGIOMR AS OMR_INGIOMR FROM <dbschema>.OMR OMR", storageOption, inheritanceType, repositoryName, modelName);
	...
	private static VSMetaTable getTableForObjRetQuery(VSMetaTable t) {
		...
		VSMetaTable newMetaTable = new VSMetaTable("OMR");
	...
	public String getPackageName() {
	   ...
	   //protected void handlers
		...
		protected void OMR_afterCommit(Session session) {
			
		}

		protected void OMR_afterDelete(DataObject obj) {

		}

		protected void OMR_afterInsert(DataObject obj) {

		}

		protected void OMR_afterRollback(Session session) {

		}

		protected void OMR_afterUpdate(DataObject obj) {

		}

		protected void OMR_beforeCommit(Session session, Response resp) {

		}

		protected void OMR_beforeDelete(DataObject obj, Response resp) {

		}

		protected void OMR_beforeInsert(DataObject obj, Response resp) {

		}

		protected void OMR_beforeRollback(Session session) {

		}

		protected void OMR_beforeUpdate(DataObject obj, Response resp) {

		}

		protected void OMR_beginTrans(Session session) {
	   ...
	   protected void splitLogic(List listOfBOs, List childAttributesIhaveToHandle, List attributesThatWillBeHandledByMyChild) {		    
			if (storageOption == com.versata.tl.vls.core.StorageOptions.STORE_ALONE)        {
			    ...
			    OMRImpl implInstance = new OMRImpl();
...
/**	  
* <br>
* Constructor for the class OMRBaseImpl.
...
*/
public OMRBaseImpl(Session session, boolean makeDefaults) {
	super(session, OMRBaseImpl.getMetaQuery(), makeDefaults);
    ...
    public OMRBaseImpl(Session session, VSMetaQuery metaQuery, boolean makeDefaults) {
...
*/
	...
	com.versata.tl.vls.admin.AppObjectImpl appObject = com.versata.tl.vls.admin.AppObjectImpl.getBusinessObject( repositoryName, "OMR", ServerEnvironment.getServer().getInternalSession());
	...
	initialized = true;
 	if (logger.isDebugEnabled())
 		logger.debug("OMR cache size: " + cacheSize + " expiration " + expires);
}
...
throws ServerException
{
	...
	if ( aSession.getSecurityCheck() ) {
		try {
		if (!aSession.getMyPrivilegeToObjectName(DataConst.AppObjectPrivilegeImpl_READ, "OMR", DataConst.AppObjectTypeCodeImpl_BUSINESS_OBJECT)) {
		throw new ServerException(ErrorMsgCode.VSMSG_SecurityNoReadAccess,  new Object[] {"business", "OMR"});
		}
		}
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e = aSession.getTransactionInfo().getObjects(OMRBaseImpl.getSubObjectRetrievalMetaQuery(), xdac, searchReq);

		Vector dObjs = new Vector();
		while(e.hasMoreElements()) {
			OMRImpl implObj = (OMRImpl) e.nextElement();
			if(implObj == null) {
				continue;
			}
		...
		ResultSet rs = new VSORBResultSetImpl( OMRBaseImpl.getSubObjectRetrievalMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
			OMRImpl bo = (OMRImpl) row.getComponent();
			if(bo == null) {
				continue;
			}
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e= aSession.getTransactionInfo().getObjects(OMRBaseImpl.getMetaQuery(), xdac, searchReq);
		Vector boList = new Vector();
		while ( e.hasMoreElements()) {
			OMRImpl bo = (OMRImpl) e.nextElement();
		...
		if (cacheSize!=0)
		{
			CacheAgent.getCacheAgent().setObjects( OMRBaseImpl.getMetaQuery(), boList, expires );
		}
		return boList.elements();		
	} else {
		Vector boList = new Vector();
		Connection con = aSession.getConnection( xdac, true );
		ResultSet rs = new VSORBResultSetImpl( OMRBaseImpl.getMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
...
public static void raiseBeforeQueryEvent( SearchRequest searchReq, Session aSession )	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "OMR.BeforeQuery") : null;
        try {
            beforeQuery( "OMR", searchReq, null , aSession );
        }
	finally {
	    if (  tr != null ) tr.end();
        }
...
public static void raiseAfterQueryEvent( ResultSet rs)	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "OMR.AfterQuery") : null;
        try {
	    afterQuery( rs );
        }
	finally {
	...
	throws ServerException
{
	...
	if(useSubObjSQL){
		OMRImpl implObj = (OMRImpl) getObjectByKeyUsingSubObjQueryInternal(key, aSession);
		if(implObj == null) {
			return null;
		}
		String tiAttrColData = implObj.get_VSTypeIndicatorAttrColumnData();
		...
		OMRImpl implObj = (OMRImpl) getObjectByKeyInternal(key, aSession, cacheOnly);
		if(implObj == null) {
			return null;
		}
	...
	throws ServerException
{
	...
	if ( cacheSize != 0 )    {
		anObject = CacheAgent.getCacheAgent().getObject( OMRBaseImpl.getMetaQuery(),key.parameterList, aSession);
		if ( logger.isDebugEnabled() ) {
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(OMRBaseImpl.getMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(OMRBaseImpl.getMetaQuery(),key);
	}
	
	// if we have a cache and got here, we had a cache miss so add this object to the global cache
	if ( cacheSize != 0 )
		 CacheAgent.getCacheAgent().addObject( OMRBaseImpl.getMetaQuery(),anObject, expires );
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(OMRBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(OMRBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	}
...
protected void insert_checkPrimaryKeyChanges() throws ServerException
{
    ...
    int pkChangeCount = 0;
    if (isChanged("OMR_OMR")) {
        pkChangeCount++;
        }       
    if (pkChangeCount > 0) {
        getSession().getTransactionInfo().save(this, "OMR");
    }
}
...
public void invalidateNonTransactionCaches() 
{
	...
	/**	  
	* <br>
	* method to get the OMR_OMR attribute for the OMR
	* @return String : the  value of the attribute OMR_OMR as String.
	*/
	public String	getOMR_OMR() 
	{
		return getData("OMR_OMR").getString();
	}
	/**	  
	* <br>
	* method to set the OMR_OMR attribute for the OMR
	* @param value String : value of the attribute OMR_OMR as String.
	*/
	public void	setOMR_OMR(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OMR_OMR");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OMR_OMR attribute for the OMR
	* @return String : the  value of the old attribute OMR_OMR as String.
	*/
	public String	getOldOMR_OMR()
	{
		return getData("OMR_OMR").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the OMR_BESK attribute for the OMR
	* @return String : the  value of the attribute OMR_BESK as String.
	*/
	public String	getOMR_BESK() 
	{
		return getData("OMR_BESK").getString();
	}
	/**	  
	* <br>
	* method to set the OMR_BESK attribute for the OMR
	* @param value String : value of the attribute OMR_BESK as String.
	*/
	public void	setOMR_BESK(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OMR_BESK");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OMR_BESK attribute for the OMR
	* @return String : the  value of the old attribute OMR_BESK as String.
	*/
	public String	getOldOMR_BESK()
	{
		return getData("OMR_BESK").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the OMR_INGIOMR attribute for the OMR
	* @return String : the  value of the attribute OMR_INGIOMR as String.
	*/
	public String	getOMR_INGIOMR() 
	{
		return getData("OMR_INGIOMR").getString();
	}
	/**	  
	* <br>
	* method to set the OMR_INGIOMR attribute for the OMR
	* @param value String : value of the attribute OMR_INGIOMR as String.
	*/
	public void	setOMR_INGIOMR(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OMR_INGIOMR");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OMR_INGIOMR attribute for the OMR
	* @return String : the  value of the old attribute OMR_INGIOMR as String.
	*/
	public String	getOldOMR_INGIOMR()
	{
		return getData("OMR_INGIOMR").getPreviousString();
	}
...
throws ServerException
	...
	if ( aSession.isTransactionInProgress())
		count = aSession.getTransactionInfo().getObjectsCount(OMRBaseImpl.getMetaQuery(), xdac, searchReq);
	else
	{
		con = aSession.getConnection( xdac, true );
  	  	count = xdac.getRowCount( OMRBaseImpl.getMetaQuery(), searchReq, con );
	}
...
{ 
	...
	try {
		VSMetaTable table = OMRBaseImpl.getMetaQuery().getChildMostTable();
		if ( table != null )
...
{ 
	...
	Properties props = ((VSORBSessionImpl)aSession).getMyDataServiceLoginForObject("OMR");
