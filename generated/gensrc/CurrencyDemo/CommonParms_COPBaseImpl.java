...
*  
*/
 public abstract class CommonParms_COPBaseImpl extends DataObject {
    
    public static VersataLogger logger = VersataLogger.getLogger(CommonParms_COPImpl.class);
    
    protected CommonParms_COPBaseImpl() {
			addListeners();
	...
	private static String modelName = "CommonParms_COP";
	...
	static {	
			...
			logger.debug("Initializing CommonParms_COP");
		
		vsQueryDefinition= new VSQueryDefinition( "CommonParms_COP" );
		...
		VSMetaColumn c = null;
		VSMetaTable tableForSubObjRetQry = new VSMetaTable("CommonParms_COP");
		
		//{{META_QUERY_COLUMN_CTOR
		
		
		
			c = new VSMetaColumn("COP_Id", DataConst.BIGINT);
			...
			c = new VSMetaColumn("COP_Description", DataConst.LONGVARCHAR);
			...
			c = new VSMetaColumn("COP_Dummy", DataConst.VARCHAR);
			c.setColumnClassification(com.versata.tl.common.meta.VSMetaColumn.LOCAL);
			c.setSize(50);
			c.setAlterability(true);
			c.setNullability(VSMetaColumn.columnNullable);
		...
		t.addUniqueKeyColumn( "CommonParms_COP Unique Key", "COP_Id" );
		  tableForSubObjRetQry.addUniqueKeyColumn( "CommonParms_COP Unique Key", "COP_Id" );
...
// end  support for key generation
		...
		if(!"VLSJSFWebConsole".equals("CurrencyDemo"))
			qTemp = (VSQueryDefinition)getMetaQuery( "CommonParms_COP","CurrencyDemo" );			
							
		if ( qTemp == null ) {			
			vsQueryDefinition.setPartialSQLForDataModel("SELECT CommonParms_COP.COP_Id AS COP_Id, CommonParms_COP.COP_Description AS COP_Description, CommonParms_COP.COP_Dummy AS COP_Dummy FROM <dbschema>.CommonParms_COP CommonParms_COP", storageOption, inheritanceType, repositoryName, modelName);
	...
	private static VSMetaTable getTableForObjRetQuery(VSMetaTable t) {
		...
		VSMetaTable newMetaTable = new VSMetaTable("CommonParms_COP");
	...
	public String getPackageName() {
	   ...
	   //protected void handlers
		...
		protected void CommonParms_COP_afterCommit(Session session) {
			
		}

		protected void CommonParms_COP_afterDelete(DataObject obj) {

		}

		protected void CommonParms_COP_afterInsert(DataObject obj) {

		}

		protected void CommonParms_COP_afterRollback(Session session) {

		}

		protected void CommonParms_COP_afterUpdate(DataObject obj) {

		}

		protected void CommonParms_COP_beforeCommit(Session session, Response resp) {

		}

		protected void CommonParms_COP_beforeDelete(DataObject obj, Response resp) {

		}

		protected void CommonParms_COP_beforeInsert(DataObject obj, Response resp) {

		}

		protected void CommonParms_COP_beforeRollback(Session session) {
		...
		protected void CommonParms_COP_beginTrans(Session session) {
	   ...
	   protected void splitLogic(List listOfBOs, List childAttributesIhaveToHandle, List attributesThatWillBeHandledByMyChild) {		    
			if (storageOption == com.versata.tl.vls.core.StorageOptions.STORE_ALONE)        {
			    ...
			    CommonParms_COPImpl implInstance = new CommonParms_COPImpl();
...
/**	  
* <br>
* Constructor for the class CommonParms_COPBaseImpl.
...
*/
public CommonParms_COPBaseImpl(Session session, boolean makeDefaults) {
	super(session, CommonParms_COPBaseImpl.getMetaQuery(), makeDefaults);
    ...
    public CommonParms_COPBaseImpl(Session session, VSMetaQuery metaQuery, boolean makeDefaults) {
...
*/
	...
	com.versata.tl.vls.admin.AppObjectImpl appObject = com.versata.tl.vls.admin.AppObjectImpl.getBusinessObject( repositoryName, "CommonParms_COP", ServerEnvironment.getServer().getInternalSession());
	...
	initialized = true;
 	if (logger.isDebugEnabled())
 		logger.debug("CommonParms_COP cache size: " + cacheSize + " expiration " + expires);
}
...
throws ServerException
{
	...
	if ( aSession.getSecurityCheck() ) {
		try {
		if (!aSession.getMyPrivilegeToObjectName(DataConst.AppObjectPrivilegeImpl_READ, "CommonParms_COP", DataConst.AppObjectTypeCodeImpl_BUSINESS_OBJECT)) {
		throw new ServerException(ErrorMsgCode.VSMSG_SecurityNoReadAccess,  new Object[] {"business", "CommonParms_COP"});
		}
		}
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e = aSession.getTransactionInfo().getObjects(CommonParms_COPBaseImpl.getSubObjectRetrievalMetaQuery(), xdac, searchReq);

		Vector dObjs = new Vector();
		while(e.hasMoreElements()) {
			CommonParms_COPImpl implObj = (CommonParms_COPImpl) e.nextElement();
			if(implObj == null) {
				continue;
			}
		...
		ResultSet rs = new VSORBResultSetImpl( CommonParms_COPBaseImpl.getSubObjectRetrievalMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
			CommonParms_COPImpl bo = (CommonParms_COPImpl) row.getComponent();
			if(bo == null) {
				continue;
			}
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e= aSession.getTransactionInfo().getObjects(CommonParms_COPBaseImpl.getMetaQuery(), xdac, searchReq);
		Vector boList = new Vector();
		while ( e.hasMoreElements()) {
			CommonParms_COPImpl bo = (CommonParms_COPImpl) e.nextElement();
		...
		if (cacheSize!=0)
		{
			CacheAgent.getCacheAgent().setObjects( CommonParms_COPBaseImpl.getMetaQuery(), boList, expires );
		}
		return boList.elements();		
	} else {
		Vector boList = new Vector();
		Connection con = aSession.getConnection( xdac, true );
		ResultSet rs = new VSORBResultSetImpl( CommonParms_COPBaseImpl.getMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
...
public static void raiseBeforeQueryEvent( SearchRequest searchReq, Session aSession )	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "CommonParms_COP.BeforeQuery") : null;
        try {
            beforeQuery( "CommonParms_COP", searchReq, null , aSession );
        }
	finally {
	    if (  tr != null ) tr.end();
        }
...
public static void raiseAfterQueryEvent( ResultSet rs)	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "CommonParms_COP.AfterQuery") : null;
        try {
	    afterQuery( rs );
        }
	finally {
	...
	throws ServerException
{
	...
	if(useSubObjSQL){
		CommonParms_COPImpl implObj = (CommonParms_COPImpl) getObjectByKeyUsingSubObjQueryInternal(key, aSession);
		if(implObj == null) {
			return null;
		}
		String tiAttrColData = implObj.get_VSTypeIndicatorAttrColumnData();
		...
		CommonParms_COPImpl implObj = (CommonParms_COPImpl) getObjectByKeyInternal(key, aSession, cacheOnly);
		if(implObj == null) {
			return null;
		}
	...
	throws ServerException
{
	...
	if ( cacheSize != 0 )    {
		anObject = CacheAgent.getCacheAgent().getObject( CommonParms_COPBaseImpl.getMetaQuery(),key.parameterList, aSession);
		if ( logger.isDebugEnabled() ) {
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(CommonParms_COPBaseImpl.getMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(CommonParms_COPBaseImpl.getMetaQuery(),key);
	}
	
	// if we have a cache and got here, we had a cache miss so add this object to the global cache
	if ( cacheSize != 0 )
		 CacheAgent.getCacheAgent().addObject( CommonParms_COPBaseImpl.getMetaQuery(),anObject, expires );
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(CommonParms_COPBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(CommonParms_COPBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	}
...
protected void insert_setCounterAttrs_replCase() throws ServerException
{
			...
			XDAConnector xdac = getXDAConnector();
				...
				if ( xdac instanceof XDASQLConnector ) {
					if ( ( counter = ((XDASQLConnector)xdac).getCounter(CommonParms_COPBaseImpl.getMetaQuery().getBaseTable("CommonParms_COP"), 
					   session.getTransactionInfo().getConnection( xdac), newRow,"COP_Id" ) )!= null )	
					   this.setCOP_Id( ((Long)counter).longValue());
					else {
							Data newData = newRow.getData("COP_Id");
							if(newData == null || newData.getValue() == null){
								throw new ServerException(ErrorMsgCode.VSMSG_IdentityColumnValueNotSet, "COP_Id");
							}							
							
							//since this is an auto-increment column, we need to ensure that the modified flag is not set(for DBs not using sequence)
							getData("COP_Id").setObject(newData.getValue());
							
					}
				}
				else {
					if ( ( counter = xdac.getCounter(CommonParms_COPBaseImpl.getMetaQuery().getBaseTable("CommonParms_COP"), 
...
protected void insert_setCounterAttrs() throws ServerException
{
			...
			XDAConnector xdac = getXDAConnector();
				...
				if ( xdac instanceof XDASQLConnector ) {
					if ( ( counter = ((XDASQLConnector)xdac).getCounter(CommonParms_COPBaseImpl.getMetaQuery().getBaseTable("CommonParms_COP"), 
					   session.getTransactionInfo().getConnection( xdac), getRow(),"COP_Id" ) )!= null )	
					   this.setCOP_Id( ((Long)counter).longValue());
...
protected void insert_checkPrimaryKeyChanges() throws ServerException
{
    ...
    if (pkChangeCount > 0) {
        getSession().getTransactionInfo().save(this, "CommonParms_COP");
    }
}
...
public void invalidateNonTransactionCaches() 
{
	...
	/**	  
	* <br>
	* method to get the COP_Description attribute for the CommonParms_COP
	* @return String : the  value of the attribute COP_Description as String.
	*/
	public String	getCOP_Description() 
	{
		return getData("COP_Description").getString();
	}
	/**	  
	* <br>
	* method to set the COP_Description attribute for the CommonParms_COP
	* @param value String : value of the attribute COP_Description as String.
	*/
	public void	setCOP_Description(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("COP_Description");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old COP_Description attribute for the CommonParms_COP
	* @return String : the  value of the old attribute COP_Description as String.
	*/
	public String	getOldCOP_Description()
	{
		return getData("COP_Description").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the COP_Dummy attribute for the CommonParms_COP
	* @return String : the  value of the attribute COP_Dummy as String.
	*/
	public String	getCOP_Dummy() 
	{
		return getData("COP_Dummy").getString();
	}
	/**	  
	* <br>
	* method to set the COP_Dummy attribute for the CommonParms_COP
	* @param value String : value of the attribute COP_Dummy as String.
	*/
	public void	setCOP_Dummy(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("COP_Dummy");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old COP_Dummy attribute for the CommonParms_COP
	* @return String : the  value of the old attribute COP_Dummy as String.
	*/
	public String	getOldCOP_Dummy()
	{
		return getData("COP_Dummy").getPreviousString();
	}
		/**	  
		* <br>
		* method to get the COP_Id attribute for the CommonParms_COP
		* Description: 
		* @return long : the  value of the attribute COP_Id as long.
		*/
		public long	getCOP_Id() 
		{
			return getData("COP_Id").getlong();
		}

		/**	  
		* <br>
		* method to set the COP_Id attribute for the CommonParms_COP
		* @param value long : value of the attribute COP_Id as long.
		*/
		public void	setCOP_Id(long value)
		{
			// The code to do convertion from the primitive data
			// to the one which can be stored goes here.
			Data dataVal = getData("COP_Id");
			dataVal.setlong(value);
		}

		/**	  
		* <br>
		* method to get the old COP_Id attribute for the CommonParms_COP
		* @return long : the  value of the old attribute COP_Id as long.
		*/
		public long	getOldCOP_Id()
		{
			return getData("COP_Id").getPreviouslong();
		}
...
throws ServerException
	...
	if ( aSession.isTransactionInProgress())
		count = aSession.getTransactionInfo().getObjectsCount(CommonParms_COPBaseImpl.getMetaQuery(), xdac, searchReq);
	else
	{
		con = aSession.getConnection( xdac, true );
  	  	count = xdac.getRowCount( CommonParms_COPBaseImpl.getMetaQuery(), searchReq, con );
	}
...
{ 
	...
	try {
		VSMetaTable table = CommonParms_COPBaseImpl.getMetaQuery().getChildMostTable();
		if ( table != null )
...
{ 
	...
	Properties props = ((VSORBSessionImpl)aSession).getMyDataServiceLoginForObject("CommonParms_COP");
...
public void RecomputeDerivations()
{
	...
	public abstract void methodReadingObjects() throws Throwable;
