...
*  
*/
 public abstract class OMR1BaseImpl extends DataObject {
    
    public static VersataLogger logger = VersataLogger.getLogger(OMR1Impl.class);
    
    protected OMR1BaseImpl() {
			addListeners();
	...
	private static String modelName = "OMR1";
	...
	static {	
			...
			logger.debug("Initializing OMR1");
		
		vsQueryDefinition= new VSQueryDefinition( "OMR1" );
		...
		VSMetaColumn c = null;
		VSMetaTable tableForSubObjRetQry = new VSMetaTable("OMR1");
		
		//{{META_QUERY_COLUMN_CTOR
		
		
		
			c = new VSMetaColumn("OMR1_OMR", DataConst.VARCHAR);
			...
			c = new VSMetaColumn("OMR1_BESK", DataConst.VARCHAR);
			...
			c = new VSMetaColumn("OMR1_INGIOMR", DataConst.VARCHAR);
			c.setColumnClassification(com.versata.tl.common.meta.VSMetaColumn.LOCAL);
			c.setSize(6);
			c.setAlterability(true);
			c.setNullability(VSMetaColumn.columnNullable);
		...
		t.addUniqueKeyColumn( "OMR1 Unique Key", "OMR1_OMR" );
		  tableForSubObjRetQry.addUniqueKeyColumn( "OMR1 Unique Key", "OMR1_OMR" );
...
// end  support for key generation
		...
		if(!"VLSJSFWebConsole".equals("CurrencyDemo"))
			qTemp = (VSQueryDefinition)getMetaQuery( "OMR1","CurrencyDemo" );			
							
		if ( qTemp == null ) {			
			vsQueryDefinition.setPartialSQLForDataModel("SELECT OMR1.OMR1_OMR AS OMR1_OMR, OMR1.OMR1_BESK AS OMR1_BESK, OMR1.OMR1_INGIOMR AS OMR1_INGIOMR FROM <dbschema>.OMR1 OMR1", storageOption, inheritanceType, repositoryName, modelName);
	...
	private static VSMetaTable getTableForObjRetQuery(VSMetaTable t) {
		...
		VSMetaTable newMetaTable = new VSMetaTable("OMR1");
	...
	public String getPackageName() {
	   ...
	   //protected void handlers
		...
		protected void OMR1_afterCommit(Session session) {
			
		}

		protected void OMR1_afterDelete(DataObject obj) {

		}

		protected void OMR1_afterInsert(DataObject obj) {

		}

		protected void OMR1_afterRollback(Session session) {

		}

		protected void OMR1_afterUpdate(DataObject obj) {

		}

		protected void OMR1_beforeCommit(Session session, Response resp) {

		}

		protected void OMR1_beforeDelete(DataObject obj, Response resp) {

		}

		protected void OMR1_beforeInsert(DataObject obj, Response resp) {

		}

		protected void OMR1_beforeRollback(Session session) {

		}

		protected void OMR1_beforeUpdate(DataObject obj, Response resp) {

		}

		protected void OMR1_beginTrans(Session session) {
	   ...
	   protected void splitLogic(List listOfBOs, List childAttributesIhaveToHandle, List attributesThatWillBeHandledByMyChild) {		    
			if (storageOption == com.versata.tl.vls.core.StorageOptions.STORE_ALONE)        {
			    ...
			    OMR1Impl implInstance = new OMR1Impl();
...
/**	  
* <br>
* Constructor for the class OMR1BaseImpl.
...
*/
public OMR1BaseImpl(Session session, boolean makeDefaults) {
	super(session, OMR1BaseImpl.getMetaQuery(), makeDefaults);
    ...
    public OMR1BaseImpl(Session session, VSMetaQuery metaQuery, boolean makeDefaults) {
...
*/
	...
	com.versata.tl.vls.admin.AppObjectImpl appObject = com.versata.tl.vls.admin.AppObjectImpl.getBusinessObject( repositoryName, "OMR1", ServerEnvironment.getServer().getInternalSession());
	...
	initialized = true;
 	if (logger.isDebugEnabled())
 		logger.debug("OMR1 cache size: " + cacheSize + " expiration " + expires);
}
...
throws ServerException
{
	...
	if ( aSession.getSecurityCheck() ) {
		try {
		if (!aSession.getMyPrivilegeToObjectName(DataConst.AppObjectPrivilegeImpl_READ, "OMR1", DataConst.AppObjectTypeCodeImpl_BUSINESS_OBJECT)) {
		throw new ServerException(ErrorMsgCode.VSMSG_SecurityNoReadAccess,  new Object[] {"business", "OMR1"});
		}
		}
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e = aSession.getTransactionInfo().getObjects(OMR1BaseImpl.getSubObjectRetrievalMetaQuery(), xdac, searchReq);

		Vector dObjs = new Vector();
		while(e.hasMoreElements()) {
			OMR1Impl implObj = (OMR1Impl) e.nextElement();
			if(implObj == null) {
				continue;
			}
		...
		ResultSet rs = new VSORBResultSetImpl( OMR1BaseImpl.getSubObjectRetrievalMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
			OMR1Impl bo = (OMR1Impl) row.getComponent();
			if(bo == null) {
				continue;
			}
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e= aSession.getTransactionInfo().getObjects(OMR1BaseImpl.getMetaQuery(), xdac, searchReq);
		Vector boList = new Vector();
		while ( e.hasMoreElements()) {
			OMR1Impl bo = (OMR1Impl) e.nextElement();
		...
		if (cacheSize!=0)
		{
			CacheAgent.getCacheAgent().setObjects( OMR1BaseImpl.getMetaQuery(), boList, expires );
		}
		return boList.elements();		
	} else {
		Vector boList = new Vector();
		Connection con = aSession.getConnection( xdac, true );
		ResultSet rs = new VSORBResultSetImpl( OMR1BaseImpl.getMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
...
public static void raiseBeforeQueryEvent( SearchRequest searchReq, Session aSession )	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "OMR1.BeforeQuery") : null;
        try {
            beforeQuery( "OMR1", searchReq, null , aSession );
        }
	finally {
	    if (  tr != null ) tr.end();
        }
...
public static void raiseAfterQueryEvent( ResultSet rs)	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "OMR1.AfterQuery") : null;
        try {
	    afterQuery( rs );
        }
	finally {
	...
	throws ServerException
{
	...
	if(useSubObjSQL){
		OMR1Impl implObj = (OMR1Impl) getObjectByKeyUsingSubObjQueryInternal(key, aSession);
		if(implObj == null) {
			return null;
		}
		String tiAttrColData = implObj.get_VSTypeIndicatorAttrColumnData();
		...
		OMR1Impl implObj = (OMR1Impl) getObjectByKeyInternal(key, aSession, cacheOnly);
		if(implObj == null) {
			return null;
		}
	...
	throws ServerException
{
	...
	if ( cacheSize != 0 )    {
		anObject = CacheAgent.getCacheAgent().getObject( OMR1BaseImpl.getMetaQuery(),key.parameterList, aSession);
		if ( logger.isDebugEnabled() ) {
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(OMR1BaseImpl.getMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(OMR1BaseImpl.getMetaQuery(),key);
	}
	
	// if we have a cache and got here, we had a cache miss so add this object to the global cache
	if ( cacheSize != 0 )
		 CacheAgent.getCacheAgent().addObject( OMR1BaseImpl.getMetaQuery(),anObject, expires );
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(OMR1BaseImpl.getSubObjectRetrievalMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(OMR1BaseImpl.getSubObjectRetrievalMetaQuery(),key);
	}
...
protected void insert_checkPrimaryKeyChanges() throws ServerException
{
    ...
    int pkChangeCount = 0;
    if (isChanged("OMR1_OMR")) {
        pkChangeCount++;
        }       
    if (pkChangeCount > 0) {
        getSession().getTransactionInfo().save(this, "OMR1");
    }
}
...
public void invalidateNonTransactionCaches() 
{
	...
	/**	  
	* <br>
	* method to get the OMR1_OMR attribute for the OMR1
	* @return String : the  value of the attribute OMR1_OMR as String.
	*/
	public String	getOMR1_OMR() 
	{
		return getData("OMR1_OMR").getString();
	}
	/**	  
	* <br>
	* method to set the OMR1_OMR attribute for the OMR1
	* @param value String : value of the attribute OMR1_OMR as String.
	*/
	public void	setOMR1_OMR(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OMR1_OMR");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OMR1_OMR attribute for the OMR1
	* @return String : the  value of the old attribute OMR1_OMR as String.
	*/
	public String	getOldOMR1_OMR()
	{
		return getData("OMR1_OMR").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the OMR1_BESK attribute for the OMR1
	* @return String : the  value of the attribute OMR1_BESK as String.
	*/
	public String	getOMR1_BESK() 
	{
		return getData("OMR1_BESK").getString();
	}
	/**	  
	* <br>
	* method to set the OMR1_BESK attribute for the OMR1
	* @param value String : value of the attribute OMR1_BESK as String.
	*/
	public void	setOMR1_BESK(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OMR1_BESK");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OMR1_BESK attribute for the OMR1
	* @return String : the  value of the old attribute OMR1_BESK as String.
	*/
	public String	getOldOMR1_BESK()
	{
		return getData("OMR1_BESK").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the OMR1_INGIOMR attribute for the OMR1
	* @return String : the  value of the attribute OMR1_INGIOMR as String.
	*/
	public String	getOMR1_INGIOMR() 
	{
		return getData("OMR1_INGIOMR").getString();
	}
	/**	  
	* <br>
	* method to set the OMR1_INGIOMR attribute for the OMR1
	* @param value String : value of the attribute OMR1_INGIOMR as String.
	*/
	public void	setOMR1_INGIOMR(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OMR1_INGIOMR");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OMR1_INGIOMR attribute for the OMR1
	* @return String : the  value of the old attribute OMR1_INGIOMR as String.
	*/
	public String	getOldOMR1_INGIOMR()
	{
		return getData("OMR1_INGIOMR").getPreviousString();
	}
...
throws ServerException
	...
	if ( aSession.isTransactionInProgress())
		count = aSession.getTransactionInfo().getObjectsCount(OMR1BaseImpl.getMetaQuery(), xdac, searchReq);
	else
	{
		con = aSession.getConnection( xdac, true );
  	  	count = xdac.getRowCount( OMR1BaseImpl.getMetaQuery(), searchReq, con );
	}
...
{ 
	...
	try {
		VSMetaTable table = OMR1BaseImpl.getMetaQuery().getChildMostTable();
		if ( table != null )
...
{ 
	...
	Properties props = ((VSORBSessionImpl)aSession).getMyDataServiceLoginForObject("OMR1");
