...
*  
*/
 public abstract class User_USRBaseImpl extends DataObject {
    
    public static VersataLogger logger = VersataLogger.getLogger(User_USRImpl.class);
    
    protected User_USRBaseImpl() {
			addListeners();
	...
	private static String modelName = "User_USR";
	...
	static {	
			...
			logger.debug("Initializing User_USR");
		
		vsQueryDefinition= new VSQueryDefinition( "User_USR" );
		...
		VSMetaColumn c = null;
		VSMetaTable tableForSubObjRetQry = new VSMetaTable("User_USR");
		
		//{{META_QUERY_COLUMN_CTOR
		
		
		
			c = new VSMetaColumn("USR_Rsn", DataConst.BIGINT);
			...
			c.setCaption("User_SeqNumber");
			tableForSubObjRetQry.addColumn( c );
			
    	
			c = new VSMetaColumn("USR_Login", DataConst.VARCHAR);
			...
			c = new VSMetaColumn("USR_Password", DataConst.VARCHAR);
			c.setColumnClassification(com.versata.tl.common.meta.VSMetaColumn.LOCAL);
			c.setSize(50);
			c.setAlterability(true);
			c.setNullability(VSMetaColumn.columnNullable);
			c.setCaption("Password");
			tableForSubObjRetQry.addColumn( c );
			
    	
			c = new VSMetaColumn("USR_Role", DataConst.VARCHAR);
			c.setColumnClassification(com.versata.tl.common.meta.VSMetaColumn.LOCAL);
			c.setSize(50);
			c.setAlterability(true);
			c.setNullability(VSMetaColumn.columnNullable);
		...
		t.addUniqueKeyColumn( "User_USR Unique Key", "USR_Rsn" );
		  tableForSubObjRetQry.addUniqueKeyColumn( "User_USR Unique Key", "USR_Rsn" );
...
// end  support for key generation
		...
		if(!"VLSJSFWebConsole".equals("CurrencyDemo"))
			qTemp = (VSQueryDefinition)getMetaQuery( "User_USR","CurrencyDemo" );			
							
		if ( qTemp == null ) {			
			vsQueryDefinition.setPartialSQLForDataModel("SELECT User_USR.USR_Rsn AS USR_Rsn, User_USR.USR_Login AS USR_Login, User_USR.USR_Password AS USR_Password, User_USR.USR_Role AS USR_Role FROM <dbschema>.User_USR User_USR", storageOption, inheritanceType, repositoryName, modelName);
	...
	private static VSMetaTable getTableForObjRetQuery(VSMetaTable t) {
		...
		VSMetaTable newMetaTable = new VSMetaTable("User_USR");
	...
	public String getPackageName() {
	   ...
	   //protected void handlers
		...
		protected void User_USR_afterCommit(Session session) {
			
		}

		protected void User_USR_afterDelete(DataObject obj) {

		}

		protected void User_USR_afterInsert(DataObject obj) {

		}

		protected void User_USR_afterRollback(Session session) {

		}

		protected void User_USR_afterUpdate(DataObject obj) {

		}

		protected void User_USR_beforeCommit(Session session, Response resp) {

		}

		protected void User_USR_beforeDelete(DataObject obj, Response resp) {

		}

		protected void User_USR_beforeInsert(DataObject obj, Response resp) {

		}

		protected void User_USR_beforeRollback(Session session) {

		}

		protected void User_USR_beforeUpdate(DataObject obj, Response resp) {

		}

		protected void User_USR_beginTrans(Session session) {
	   ...
	   protected void splitLogic(List listOfBOs, List childAttributesIhaveToHandle, List attributesThatWillBeHandledByMyChild) {		    
			if (storageOption == com.versata.tl.vls.core.StorageOptions.STORE_ALONE)        {
			    ...
			    User_USRImpl implInstance = new User_USRImpl();
...
/**	  
* <br>
* Constructor for the class User_USRBaseImpl.
...
*/
public User_USRBaseImpl(Session session, boolean makeDefaults) {
	super(session, User_USRBaseImpl.getMetaQuery(), makeDefaults);
    ...
    public User_USRBaseImpl(Session session, VSMetaQuery metaQuery, boolean makeDefaults) {
...
*/
	...
	com.versata.tl.vls.admin.AppObjectImpl appObject = com.versata.tl.vls.admin.AppObjectImpl.getBusinessObject( repositoryName, "User_USR", ServerEnvironment.getServer().getInternalSession());
	...
	initialized = true;
 	if (logger.isDebugEnabled())
 		logger.debug("User_USR cache size: " + cacheSize + " expiration " + expires);
}
...
throws ServerException
{
	...
	if ( aSession.getSecurityCheck() ) {
		try {
		if (!aSession.getMyPrivilegeToObjectName(DataConst.AppObjectPrivilegeImpl_READ, "User_USR", DataConst.AppObjectTypeCodeImpl_BUSINESS_OBJECT)) {
		throw new ServerException(ErrorMsgCode.VSMSG_SecurityNoReadAccess,  new Object[] {"business", "User_USR"});
		}
		}
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e = aSession.getTransactionInfo().getObjects(User_USRBaseImpl.getSubObjectRetrievalMetaQuery(), xdac, searchReq);

		Vector dObjs = new Vector();
		while(e.hasMoreElements()) {
			User_USRImpl implObj = (User_USRImpl) e.nextElement();
			if(implObj == null) {
				continue;
			}
		...
		ResultSet rs = new VSORBResultSetImpl( User_USRBaseImpl.getSubObjectRetrievalMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
			User_USRImpl bo = (User_USRImpl) row.getComponent();
			if(bo == null) {
				continue;
			}
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e= aSession.getTransactionInfo().getObjects(User_USRBaseImpl.getMetaQuery(), xdac, searchReq);
		Vector boList = new Vector();
		while ( e.hasMoreElements()) {
			User_USRImpl bo = (User_USRImpl) e.nextElement();
		...
		if (cacheSize!=0)
		{
			CacheAgent.getCacheAgent().setObjects( User_USRBaseImpl.getMetaQuery(), boList, expires );
		}
		return boList.elements();		
	} else {
		Vector boList = new Vector();
		Connection con = aSession.getConnection( xdac, true );
		ResultSet rs = new VSORBResultSetImpl( User_USRBaseImpl.getMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
...
public static void raiseBeforeQueryEvent( SearchRequest searchReq, Session aSession )	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "User_USR.BeforeQuery") : null;
        try {
            beforeQuery( "User_USR", searchReq, null , aSession );
        }
	finally {
	    if (  tr != null ) tr.end();
        }
...
public static void raiseAfterQueryEvent( ResultSet rs)	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "User_USR.AfterQuery") : null;
        try {
	    afterQuery( rs );
        }
	finally {
	...
	throws ServerException
{
	...
	if(useSubObjSQL){
		User_USRImpl implObj = (User_USRImpl) getObjectByKeyUsingSubObjQueryInternal(key, aSession);
		if(implObj == null) {
			return null;
		}
		String tiAttrColData = implObj.get_VSTypeIndicatorAttrColumnData();
		...
		User_USRImpl implObj = (User_USRImpl) getObjectByKeyInternal(key, aSession, cacheOnly);
		if(implObj == null) {
			return null;
		}
	...
	throws ServerException
{
	...
	if ( cacheSize != 0 )    {
		anObject = CacheAgent.getCacheAgent().getObject( User_USRBaseImpl.getMetaQuery(),key.parameterList, aSession);
		if ( logger.isDebugEnabled() ) {
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(User_USRBaseImpl.getMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(User_USRBaseImpl.getMetaQuery(),key);
	}
	
	// if we have a cache and got here, we had a cache miss so add this object to the global cache
	if ( cacheSize != 0 )
		 CacheAgent.getCacheAgent().addObject( User_USRBaseImpl.getMetaQuery(),anObject, expires );
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(User_USRBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(User_USRBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	}
...
protected void insert_checkPrimaryKeyChanges() throws ServerException
{
    ...
    int pkChangeCount = 0;
    if (isChanged("USR_Rsn")) {
        pkChangeCount++;
        }       
    if (pkChangeCount > 0) {
        getSession().getTransactionInfo().save(this, "User_USR");
    }
}
...
public void invalidateNonTransactionCaches() 
{
	...
	/**	  
	* <br>
	* method to get the USR_Rsn attribute for the User_USR
	* @return long : the  value of the attribute USR_Rsn as long.
	*/
	public long	getUSR_Rsn() 
	{
		return getData("USR_Rsn").getlong();
	}
	/**	  
	* <br>
	* method to set the USR_Rsn attribute for the User_USR
	* @param value long : value of the attribute USR_Rsn as long.
	*/
	public void	setUSR_Rsn(long value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("USR_Rsn");
		dataVal.setlong(value);
	}

	/**	  
	* <br>
	* method to get the old USR_Rsn attribute for the User_USR
	* @return long : the  value of the old attribute USR_Rsn as long.
	*/
	public long	getOldUSR_Rsn()
	{
		return getData("USR_Rsn").getPreviouslong();
	}
		/**	  
	* <br>
	* method to get the USR_Login attribute for the User_USR
	* @return String : the  value of the attribute USR_Login as String.
	*/
	public String	getUSR_Login() 
	{
		return getData("USR_Login").getString();
	}
	/**	  
	* <br>
	* method to set the USR_Login attribute for the User_USR
	* @param value String : value of the attribute USR_Login as String.
	*/
	public void	setUSR_Login(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("USR_Login");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old USR_Login attribute for the User_USR
	* @return String : the  value of the old attribute USR_Login as String.
	*/
	public String	getOldUSR_Login()
	{
		return getData("USR_Login").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the USR_Password attribute for the User_USR
	* @return String : the  value of the attribute USR_Password as String.
	*/
	public String	getUSR_Password() 
	{
		return getData("USR_Password").getString();
	}
	/**	  
	* <br>
	* method to set the USR_Password attribute for the User_USR
	* @param value String : value of the attribute USR_Password as String.
	*/
	public void	setUSR_Password(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("USR_Password");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old USR_Password attribute for the User_USR
	* @return String : the  value of the old attribute USR_Password as String.
	*/
	public String	getOldUSR_Password()
	{
		return getData("USR_Password").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the USR_Role attribute for the User_USR
	* @return String : the  value of the attribute USR_Role as String.
	*/
	public String	getUSR_Role() 
	{
		return getData("USR_Role").getString();
	}
	/**	  
	* <br>
	* method to set the USR_Role attribute for the User_USR
	* @param value String : value of the attribute USR_Role as String.
	*/
	public void	setUSR_Role(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("USR_Role");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old USR_Role attribute for the User_USR
	* @return String : the  value of the old attribute USR_Role as String.
	*/
	public String	getOldUSR_Role()
	{
		return getData("USR_Role").getPreviousString();
	}
...
throws ServerException
	...
	if ( aSession.isTransactionInProgress())
		count = aSession.getTransactionInfo().getObjectsCount(User_USRBaseImpl.getMetaQuery(), xdac, searchReq);
	else
	{
		con = aSession.getConnection( xdac, true );
  	  	count = xdac.getRowCount( User_USRBaseImpl.getMetaQuery(), searchReq, con );
	}
...
{ 
	...
	try {
		VSMetaTable table = User_USRBaseImpl.getMetaQuery().getChildMostTable();
		if ( table != null )
...
{ 
	...
	Properties props = ((VSORBSessionImpl)aSession).getMyDataServiceLoginForObject("User_USR");
