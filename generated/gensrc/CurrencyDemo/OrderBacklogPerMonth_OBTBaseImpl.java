...
*  
*/
 public abstract class OrderBacklogPerMonth_OBTBaseImpl extends DataObject {
    
    public static VersataLogger logger = VersataLogger.getLogger(OrderBacklogPerMonth_OBTImpl.class);
    
    protected OrderBacklogPerMonth_OBTBaseImpl() {
			addListeners();
	...
	private static String modelName = "OrderBacklogPerMonth_OBT";
	...
	static {	
			...
			logger.debug("Initializing OrderBacklogPerMonth_OBT");
		
		vsQueryDefinition= new VSQueryDefinition( "OrderBacklogPerMonth_OBT" );
		...
		VSMetaColumn c = null;
		VSMetaTable tableForSubObjRetQry = new VSMetaTable("OrderBacklogPerMonth_OBT");
		
		//{{META_QUERY_COLUMN_CTOR
		
		
		
			c = new VSMetaColumn("OBT_Client", DataConst.VARCHAR);
			...
			c = new VSMetaColumn("OBT_Month", DataConst.CHAR);
			c.setColumnClassification(com.versata.tl.common.meta.VSMetaColumn.PK);
			c.setSize(3);
			c.setAlterability(true);
			c.setNullability(VSMetaColumn.columnNullable);
			...
			c = new VSMetaColumn("OBT_OrderValueUSD", DataConst.DECIMAL);
			...
			c.setNullability(VSMetaColumn.columnNullable);
			c.setComputed (true);
			c.setCaption("Order Value USD");
			tableForSubObjRetQry.addColumn( c );
			
    			
		VSMetaTable t = getTableForObjRetQuery(tableForSubObjRetQry);
		t.addUniqueKeyColumn( "OrderBacklogPerMonth_OBT Unique Key", "OBT_Client" );
		  tableForSubObjRetQry.addUniqueKeyColumn( "OrderBacklogPerMonth_OBT Unique Key", "OBT_Client" );
			t.addUniqueKeyColumn( "OrderBacklogPerMonth_OBT Unique Key", "OBT_Month" );
		  tableForSubObjRetQry.addUniqueKeyColumn( "OrderBacklogPerMonth_OBT Unique Key", "OBT_Month" );
	


// begin key generation
	
				/// got assoc: OrderBacklogPerMonth_OBT(pOBTfORD)-OrderItem_ORD(cORDfOBT) (enforced: true) 
				t.addUniqueKeyColumn("Parent Key: OrderBacklogPerMonth_OBT(pOBTfORD)-OrderItem_ORD(cORDfOBT)", "OBT_Client"); // <- ORD_Client
				// got keyPair: com.versata.tl.models.bl.dataobject.impl.AttributePairImpl@1364146 
					
				/// got assoc: OrderBacklogPerMonth_OBT(pOBTfORD)-OrderItem_ORD(cORDfOBT) (enforced: true) 
				t.addUniqueKeyColumn("Parent Key: OrderBacklogPerMonth_OBT(pOBTfORD)-OrderItem_ORD(cORDfOBT)", "OBT_Month"); // <- ORD_Month
				// got keyPair: com.versata.tl.models.bl.dataobject.impl.AttributePairImpl@f795c1 
...
// end  support for key generation
		...
		t.setXDAConnectorClassName("com.versata.tl.vls.admin.security.F2KConnector");
			tableForSubObjRetQry.setXDAConnectorClassName("com.versata.tl.vls.admin.security.F2KConnector");
		...
		if(!"VLSJSFWebConsole".equals("CurrencyDemo"))
			qTemp = (VSQueryDefinition)getMetaQuery( "OrderBacklogPerMonth_OBT","CurrencyDemo" );			
							
		if ( qTemp == null ) {			
			vsQueryDefinition.setPartialSQLForDataModel("SELECT OrderBacklogPerMonth_OBT.OBT_Client AS OBT_Client, OrderBacklogPerMonth_OBT.OBT_Month AS OBT_Month, OrderBacklogPerMonth_OBT.OBT_OrderValueUSD AS OBT_OrderValueUSD FROM <dbschema>.OrderBacklogPerMonth_OBT OrderBacklogPerMonth_OBT", storageOption, inheritanceType, repositoryName, modelName);
	...
	t.setDerivationType("OBT_OrderValueUSD", VSMetaColumn.derivationSum);
			tableForSubObjRetQry.setDerivationType("OBT_OrderValueUSD", VSMetaColumn.derivationSum);
	...
	private static VSMetaTable getTableForObjRetQuery(VSMetaTable t) {
		...
		VSMetaTable newMetaTable = new VSMetaTable("OrderBacklogPerMonth_OBT");
	...
	public String getPackageName() {
	   ...
	   //protected void handlers
		...
		protected void OrderBacklogPerMonth_OBT_afterCommit(Session session) {
			
		}

		protected void OrderBacklogPerMonth_OBT_afterDelete(DataObject obj) {

		}

		protected void OrderBacklogPerMonth_OBT_afterInsert(DataObject obj) {

		}

		protected void OrderBacklogPerMonth_OBT_afterRollback(Session session) {

		}

		protected void OrderBacklogPerMonth_OBT_afterUpdate(DataObject obj) {

		}

		protected void OrderBacklogPerMonth_OBT_beforeCommit(Session session, Response resp) {

		}

		protected void OrderBacklogPerMonth_OBT_beforeDelete(DataObject obj, Response resp) {

		}

		protected void OrderBacklogPerMonth_OBT_beforeInsert(DataObject obj, Response resp) {

		}

		protected void OrderBacklogPerMonth_OBT_beforeRollback(Session session) {

		}

		protected void OrderBacklogPerMonth_OBT_beforeUpdate(DataObject obj, Response resp) {

		}

		protected void OrderBacklogPerMonth_OBT_beginTrans(Session session) {
	   ...
	   protected void splitLogic(List listOfBOs, List childAttributesIhaveToHandle, List attributesThatWillBeHandledByMyChild) {		    
			if (storageOption == com.versata.tl.vls.core.StorageOptions.STORE_ALONE)        {
			    ...
			    OrderBacklogPerMonth_OBTImpl implInstance = new OrderBacklogPerMonth_OBTImpl();
...
public String getComponentName() {
	return "OrderBacklogPerMonth_OBT";
}
...
/**	  
* <br>
* Constructor for the class OrderBacklogPerMonth_OBTBaseImpl.
...
*/
public OrderBacklogPerMonth_OBTBaseImpl(Session session, boolean makeDefaults) {
	super(session, OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(), makeDefaults);
    ...
    public OrderBacklogPerMonth_OBTBaseImpl(Session session, VSMetaQuery metaQuery, boolean makeDefaults) {
...
*/
	...
	com.versata.tl.vls.admin.AppObjectImpl appObject = com.versata.tl.vls.admin.AppObjectImpl.getBusinessObject( repositoryName, "OrderBacklogPerMonth_OBT", ServerEnvironment.getServer().getInternalSession());
	...
	initialized = true;
 	if (logger.isDebugEnabled())
 		logger.debug("OrderBacklogPerMonth_OBT cache size: " + cacheSize + " expiration " + expires);
}
...
throws ServerException
{
	...
	if ( aSession.getSecurityCheck() ) {
		try {
		if (!aSession.getMyPrivilegeToObjectName(DataConst.AppObjectPrivilegeImpl_READ, "OrderBacklogPerMonth_OBT", DataConst.AppObjectTypeCodeImpl_BUSINESS_OBJECT)) {
		throw new ServerException(ErrorMsgCode.VSMSG_SecurityNoReadAccess,  new Object[] {"business", "OrderBacklogPerMonth_OBT"});
		}
		}
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e = aSession.getTransactionInfo().getObjects(OrderBacklogPerMonth_OBTBaseImpl.getSubObjectRetrievalMetaQuery(), xdac, searchReq);

		Vector dObjs = new Vector();
		while(e.hasMoreElements()) {
			OrderBacklogPerMonth_OBTImpl implObj = (OrderBacklogPerMonth_OBTImpl) e.nextElement();
			if(implObj == null) {
				continue;
			}
		...
		ResultSet rs = new VSORBResultSetImpl( OrderBacklogPerMonth_OBTBaseImpl.getSubObjectRetrievalMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
			OrderBacklogPerMonth_OBTImpl bo = (OrderBacklogPerMonth_OBTImpl) row.getComponent();
			if(bo == null) {
				continue;
			}
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e= aSession.getTransactionInfo().getObjects(OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(), xdac, searchReq);
		Vector boList = new Vector();
		while ( e.hasMoreElements()) {
			OrderBacklogPerMonth_OBTImpl bo = (OrderBacklogPerMonth_OBTImpl) e.nextElement();
		...
		if (cacheSize!=0)
		{
			CacheAgent.getCacheAgent().setObjects( OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(), boList, expires );
		}
		return boList.elements();		
	} else {
		Vector boList = new Vector();
		Connection con = aSession.getConnection( xdac, true );
		ResultSet rs = new VSORBResultSetImpl( OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
		raiseAfterQueryEvent( rs );
		DataRow row;
		while (  ( row =  rs.fetch() ) != null )
		{	
...
public static void raiseBeforeQueryEvent( SearchRequest searchReq, Session aSession )	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "OrderBacklogPerMonth_OBT.BeforeQuery") : null;
        try {
            beforeQuery( "OrderBacklogPerMonth_OBT", searchReq, null , aSession );
        }
	finally {
	    if (  tr != null ) tr.end();
        }
...
public static void raiseAfterQueryEvent( ResultSet rs)	{
        VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "OrderBacklogPerMonth_OBT.AfterQuery") : null;
        try {
	    afterQuery( rs );
        }
	finally {
	...
	throws ServerException
{
	...
	if(useSubObjSQL){
		OrderBacklogPerMonth_OBTImpl implObj = (OrderBacklogPerMonth_OBTImpl) getObjectByKeyUsingSubObjQueryInternal(key, aSession);
		if(implObj == null) {
			return null;
		}
		String tiAttrColData = implObj.get_VSTypeIndicatorAttrColumnData();
		...
		OrderBacklogPerMonth_OBTImpl implObj = (OrderBacklogPerMonth_OBTImpl) getObjectByKeyInternal(key, aSession, cacheOnly);
		if(implObj == null) {
			return null;
		}
	...
	throws ServerException
{
	...
	if ( cacheSize != 0 )    {
		anObject = CacheAgent.getCacheAgent().getObject( OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(),key.parameterList, aSession);
		if ( logger.isDebugEnabled() ) {
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(),key);
	}
	
	// if we have a cache and got here, we had a cache miss so add this object to the global cache
	if ( cacheSize != 0 )
		 CacheAgent.getCacheAgent().addObject( OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(),anObject, expires );
	...
	throws ServerException
{
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(OrderBacklogPerMonth_OBTBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(OrderBacklogPerMonth_OBTBaseImpl.getSubObjectRetrievalMetaQuery(),key);
	}
...
public void tableConditionActions() {
	// Generate code for conditional action events.
		//calling action BigOrderRule
		  tableConditionAction_BigOrderRule();
		 ...
		 * @ConditionName BigOrderRule
		 * @description  
		 * @return
		 */
		public void tableConditionAction_BigOrderRule()
		{
				
			VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, "VST_RULE_CONDITIONACTION",this.getClass().getName() + "_BigOrderRule") : null;  
			try {
				// Condition rule processing for BigOrderRule
        boolean condition = this.getOBT_OrderValueUSD().compareTo((new BigDecimal("100000"))) == 1 && this.getOldOBT_OrderValueUSD().compareTo((new BigDecimal("100000"))) == -1 ;
        if (logger.isDebugEnabled()) logger.debug("Evaluating condition BigOrderRule");
        if ( condition )
				{
					if (tr != null) {
						tr.set("VST_RULE_FIRE.CONDITIONACTION_" + this.getClass().getName() + ".BigOrderRule", "true");
	
					}
					if (isServerEventTracingEnabled()) addServerEvent("VST_RULE_FIRE.CONDITIONACTION_" + this.getClass().getName() + ".BigOrderRule" + " : " + condition, this.toString(), Session.RULE_EVENT);
					logBamEcaRule("BAM.CurrencyDemo.OrderBacklogPerMonth_OBT.RuleExecutionEvent.BigOrderRule", "BigOrderRule");
					bigOrder( );
				}
			} catch (Exception e) {
				handleActionException(e, "_BigOrderRule");				
			} finally {
				if ( tr != null ) tr.end();
			}
		}
...
protected void insert_setAggregateAttrs() throws ServerException
{
	 // Set the default values for the aggregates
		setOBT_OrderValueUSD(new BigDecimal("0"));
...
protected void insert_checkPrimaryKeyChanges() throws ServerException
{
    ...
    int pkChangeCount = 0;
    if (isChanged("OBT_Client")) {
        pkChangeCount++;
        }
    if (isChanged("OBT_Month")) {
        pkChangeCount++;
        }       
    if (pkChangeCount > 0) {
        getSession().getTransactionInfo().save(this, "OrderBacklogPerMonth_OBT");
    }
}
...
{
	// Child Cascade for Role OrderBacklogPerMonth_OBT(pOBTfORD)-OrderItem_ORD(cORDfOBT): pOBTfORD->>cORDfOBT
			this.childCascadeFor_cORDfOBT();
...
{
	if ( isChanged("OBT_OrderValueUSD") )
				getOBT_OrderValueUSD();
...
public void tableConstraintCheck()
	{
	 //calling constraint OrderAmountConstraint
              tableConstraintCheck_OrderAmountConstraint();	
		
	}

			/**
			 * <br>
			 * Rule Constraint Check method
			 * @ConstraintName OrderAmountConstraint 
			 * @description  
			 * @return
			 */
			public void tableConstraintCheck_OrderAmountConstraint() {
			VSTraceHelper tr = null;  
			try {
				if (logger.isDebugEnabled()) logger.debug("tableConstraintCheck OrderBacklogPerMonth_OBT_OrderAmountConstraint  DataObject:" + this.toString());
				
				tr =  VSTrace.IS_ON ? new VSTraceHelper(logger, VST_RULE,"tableConstraintCheck", "OrderBacklogPerMonth_OBT_OrderAmountConstraint") : null;
				
				getOBT_OrderValueUSD();
								if (this.getOBT_OrderValueUSD().compareTo((new BigDecimal("100000"))) >= 0 ) {
							logBamConstraintRule("BAM.CurrencyDemo.OrderBacklogPerMonth_OBT.OrderAmountConstraint", "OrderAmountConstraint");
								
								raiseException("Orderamaount USD >= 100000");
								
						}
				
			} catch (Exception ex) {
			 	String dependents[] = { "OBT_OrderValueUSD"
					 };
			 	
				handleConstraintValidationException(ex, "OrderAmountConstraint", dependents);
			} finally { 
				if (tr != null) tr.end();
			}
...
protected void childCascadeFor_cORDfOBT()
{
	 if (logger.isDebugEnabled()) logger.debug("childCascadeFor_cORDfOBT " + this.toString());
		VSTraceHelper tr = VSTrace.IS_ON ? new VSTraceHelper(logger, VST_RULE, "childCascadeFor_cORDfOBT", "OrderBacklogPerMonth_OBT") : null;  
		try {	
			...
			if (( isUpdated() == true ) &&( 
						(isChanged("OBT_Client")) || 	(isChanged("OBT_Month")) )) {    
					pkeyChanged = true;
				}
    		...
    		if (isDeleted())
    		{
      			...
      			children = getOldcORDfOBT(false);
      			if ( children.hasMoreElements() ) // There are children found
      			{
      					raiseException("Delete Rejected because there are OrderItem_ORD found for OrderBacklogPerMonth_OBT");
						
					   }
					     }
	else if ( pkeyChanged || replicatedAttrChanged )
		{
      boolean saveCurrentCacheStatus = cacheChildrenForcORDfOBT;
      cacheChildrenForcORDfOBT = false;  // force a cache bypass
			children = pkeyChanged ? getOldcORDfOBT() : getcORDfOBT();
      cacheChildrenForcORDfOBT = saveCurrentCacheStatus;
		...
		if ( children.hasMoreElements() ) // There are children found
			...
			if ( pkeyChanged )
						{	
							...
							raiseException("Update Rejected because there are OrderItem_ORD found for OrderBacklogPerMonth_OBT");
						 }
		...
		private boolean cacheChildrenForcORDfOBTConsistent = false;
		private boolean cacheChildrenForcORDfOBT = true;
		private boolean cacheForcORDfOBTPopulatedInTxn = false;
		private ObjectHashtable objectCacheForcORDfOBT = null;
		
		/**	  
		* <br>
		* method to retrieve the OrderItem_ORD objects for this OrderBacklogPerMonth_OBT
		...
		*/
		public Enumeration getcORDfOBT()
		{
			if (cacheChildrenForcORDfOBT && cacheChildrenForcORDfOBTConsistent)
			{
				if ( objectCacheForcORDfOBT != null ) return objectCacheForcORDfOBT.elements();
			}
			// if we're not caching OR the cache is null 
			SearchRequest searchReq = new SearchRequest();
			Parameter param;
 			param = new Parameter("OrderItem_ORD", "ORD_Client", getData("OBT_Client").getString(), repositoryName);
					  searchReq.add(param);
			param = new Parameter("OrderItem_ORD", "ORD_Month", getData("OBT_Month").getString(), repositoryName);
					  searchReq.add(param);
			
 			if (cacheChildrenForcORDfOBT)
			{
				objectCacheForcORDfOBT = new ObjectHashtable();
				cacheForcORDfOBTPopulatedInTxn = getSession().isTransactionInProgress();
				for (Enumeration e = (OrderItem_ORDBaseImpl.getObjects(searchReq ,getSession())); e.hasMoreElements();)
				{
					...
					objectCacheForcORDfOBT.put(pkey,cacheBO);								
				}
				// cacheChildrenForcORDfOBTConsistent set to true as the value is got from search request.
				cacheChildrenForcORDfOBTConsistent = true;
				return (objectCacheForcORDfOBT.elements());
			}
			else
			{
		...
		/**	  
		* <br>
		* Method to create a new instance of the OrderItem_ORD object for this OrderBacklogPerMonth_OBT
		* @return OrderItem_ORD object with the foreign key set into it.
		*/
		public OrderItem_ORDImpl newInstanceForcORDfOBT()
		{
		
			OrderItem_ORDImpl childObj = (OrderItem_ORDImpl) OrderItem_ORDImpl.getNewObject(getSession(), true); 
 			
			childObj.setORD_Client(getData("OBT_Client").getString());
			
			childObj.setORD_Month(getData("OBT_Month").getString());
...
/**	  
		* <br>
		* method to retrieve the old OrderItem_ORD objects for this  OrderBacklogPerMonth_OBT
		* Old OrderItem_ORD objects would be different from the new ones if
		* the  OrderBacklogPerMonth_OBT has a primary key change.
		...
		*/
  public Enumeration getOldcORDfOBT(boolean useCache)
  {
    boolean oldCacheValue = cacheChildrenForcORDfOBT;
    cacheChildrenForcORDfOBT = useCache;
    try {
      return getOldcORDfOBT();
    } 
    finally {                     
      cacheChildrenForcORDfOBT = oldCacheValue;
    }			
  }
    ...
    /**	  
		...
		* method to retrieve the old OrderItem_ORD objects for this  OrderBacklogPerMonth_OBT
		* old OrderItem_ORD objects would be different from the new ones if
		* the  OrderBacklogPerMonth_OBT has a primary key change.  This method will use the relationship
		* cache if applicable.
		* This method currently does not support additional conditional params.
		* @return Enumeration : the Enumeration of old OrderItem_ORD objects.
		*/
		public Enumeration	getOldcORDfOBT()
		{
			if (cacheChildrenForcORDfOBT && cacheChildrenForcORDfOBTConsistent)
			{
				if ( objectCacheForcORDfOBT != null ) return objectCacheForcORDfOBT.elements();
			...
			SearchRequest searchReq = new SearchRequest();
			Parameter param;
			param = new Parameter("OrderItem_ORD", "ORD_Client", getData("OBT_Client").getPreviousString(), repositoryName);
					  searchReq.add(param);
			param = new Parameter("OrderItem_ORD", "ORD_Month", getData("OBT_Month").getPreviousString(), repositoryName);
					  searchReq.add(param);
			...
			if (cacheChildrenForcORDfOBT)
			{
				...
				cacheChildrenForcORDfOBTConsistent = false;
				return (objectCacheForcORDfOBT.elements());
			}
			else
			{
		...
		public void updateCacheForcORDfOBT(OrderItem_ORDBaseImpl child, boolean remove)
		{
			
			if ( objectCacheForcORDfOBT == null ) {
			    objectCacheForcORDfOBT = new ObjectHashtable();
			    // cacheChildrenForcORDfOBTConsistent set to false, as the values in db are not retreived here.
			    cacheChildrenForcORDfOBTConsistent = false;
				cacheForcORDfOBTPopulatedInTxn = getSession().isTransactionInProgress();
			}
			if ( child != null ) {
				if ( remove ) {
					if ( objectCacheForcORDfOBT.containsKey(child.getRow().getPkeyParams()) )
						objectCacheForcORDfOBT.remove(child.getRow().getPkeyParams());
				}
				else {
					// Add only if it is not in the cache.
					if ( ! objectCacheForcORDfOBT.containsKey(child.getRow().getPkeyParams()) )
						objectCacheForcORDfOBT.put(child.getRow().getPkeyParams(), child);
				}
			}
		}
...
{
	  if( !cacheForcORDfOBTPopulatedInTxn )
                objectCacheForcORDfOBT = null;
}
	...
	/**	  
	* <br>
	* method to get the OBT_Client attribute for the OrderBacklogPerMonth_OBT
	* @return String : the  value of the attribute OBT_Client as String.
	*/
	public String	getOBT_Client() 
	{
		return getData("OBT_Client").getString();
	}
	/**	  
	* <br>
	* method to set the OBT_Client attribute for the OrderBacklogPerMonth_OBT
	* @param value String : value of the attribute OBT_Client as String.
	*/
	public void	setOBT_Client(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OBT_Client");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OBT_Client attribute for the OrderBacklogPerMonth_OBT
	* @return String : the  value of the old attribute OBT_Client as String.
	*/
	public String	getOldOBT_Client()
	{
		return getData("OBT_Client").getPreviousString();
	}
		/**	  
	* <br>
	* method to get the OBT_Month attribute for the OrderBacklogPerMonth_OBT
	* @return String : the  value of the attribute OBT_Month as String.
	*/
	public String	getOBT_Month() 
	{
		return getData("OBT_Month").getString();
	}
	/**	  
	* <br>
	* method to set the OBT_Month attribute for the OrderBacklogPerMonth_OBT
	* @param value String : value of the attribute OBT_Month as String.
	*/
	public void	setOBT_Month(String value)
	{
		// The code to do convertion from the primitive data
		// to the one which can be stored goes here.
		Data dataVal = getData("OBT_Month");
		dataVal.setString(value);
	}

	/**	  
	* <br>
	* method to get the old OBT_Month attribute for the OrderBacklogPerMonth_OBT
	* @return String : the  value of the old attribute OBT_Month as String.
	*/
	public String	getOldOBT_Month()
	{
		return getData("OBT_Month").getPreviousString();
	}
	
/**	  
	* <br>
	* method to get the summed attribute OBT_OrderValueUSD for the OrderBacklogPerMonth_OBT
	* Description: 
	* @return BigDecimal : the  value of the attribute OBT_OrderValueUSD as BigDecimal.
	*/
	public BigDecimal	getOBT_OrderValueUSD()
	{
		 /**@ Not a virtual @**/
			Data dataVal = getData("OBT_OrderValueUSD");
			if (dataVal.isComputationPending() ||  ( this.isCurrentEvent("RECOMPUTE_DERIVATIONS") && (getGlobalNestLevel() == 1) ))
			{
				...
				else
				{
					...
					Parameter param = null;
						...
						{
							if ((getGlobalNestLevel() == 1) && (isChanged("OBT_Client")))
								param.value = getData("OBT_Client").getOldString();
							else
								param.value = getData("OBT_Client").getString();
						}
						else
						{
							param.value = getData("OBT_Client").getString();
						...
						param = new Parameter();
						param.objName = "OrderItem_ORD";
						param.fieldName = "ORD_Month";
						if (getSession().isTransactionInProgress())
						{
							if ((getGlobalNestLevel() == 1) && (isChanged("OBT_Month")))
								param.value = getData("OBT_Month").getOldString();
							else
								param.value = getData("OBT_Month").getString();
						}
						else
						{
							param.value = getData("OBT_Month").getString();
					...
					dataVal.setBigDecimal((OrderItem_ORDBaseImpl.getOrderBacklogPerMonth_OBTOBT_OrderValueUSD(searchReq, "ORD_AmountInUSD", getSession())));
				}
			...
			/**	  
		* <br>
		* method to get the value before data change for the summed attribute OBT_OrderValueUSD
		* in the data object OrderBacklogPerMonth_OBT
		* @return BigDecimal : the  value of the attribute OBT_OrderValueUSD as BigDecimal before change.
		*/
		public BigDecimal	getOldOBT_OrderValueUSD()
		{
				return getData("OBT_OrderValueUSD").getPreviousBigDecimal();
		}

		/**	  
		* <br>
		* method to set the value of the summed attribute OBT_OrderValueUSD in the data object OrderBacklogPerMonth_OBT
		* @param value BigDecimal : value of the attribute OBT_OrderValueUSD as BigDecimal.
		*/
		public void setOBT_OrderValueUSD(BigDecimal value)
		{
			// The code to do convertion from the primitive data
			// to the one which can be stored goes here.
			Data dataVal = getData("OBT_OrderValueUSD");
...
throws ServerException
	...
	if ( aSession.isTransactionInProgress())
		count = aSession.getTransactionInfo().getObjectsCount(OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(), xdac, searchReq);
	else
	{
		con = aSession.getConnection( xdac, true );
  	  	count = xdac.getRowCount( OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery(), searchReq, con );
	}
...
{ 
	...
	try {
		VSMetaTable table = OrderBacklogPerMonth_OBTBaseImpl.getMetaQuery().getChildMostTable();
		if ( table != null )
...
{ 
	...
	Properties props = ((VSORBSessionImpl)aSession).getMyDataServiceLoginForObject("OrderBacklogPerMonth_OBT");
...
public void RecomputeDerivations()
{
			...
			if (logger.isDebugEnabled()) logger.debug("recomputing OBT_OrderValueUSD");
			this.setOBT_OrderValueUSD(getOBT_OrderValueUSD());
