 ...
 * fZ_YesNo_Grid
 */
abstract class fZ_YesNo_GridBase extends VSForm {
	...
	protected static final VersataLogger logger = VersataLogger.getLogger(JavaApp.class);
		...
		VSJavaDataControl datT1Z_YesNo = new VSJavaDataControl("datT1Z_YesNo");    
		VSGrid grdT1Z_YesNo = new VSGrid();
		VSScrollPane scrlgrdT1Z_YesNo = new VSScrollPane(grdT1Z_YesNo);
	
	//END_FORM_VAR_DECL}}

	public fZ_YesNo_GridBase() throws VSException, java.beans.PropertyVetoException {
		//{{FORM_OBJ_CTOR
			setLayout (null);
		
		beforeFormInitialize();
		
		setName("fZ_YesNo_Grid");
			...
			initVSJavaDataControldatT1Z_YesNo();
			
			initVSGridgrdT1Z_YesNo();
...
add (btnEditSql,null);             
	...
	public void initVSJavaDataControldatT1Z_YesNo() { 
	datT1Z_YesNo.setLabel ("Z_YesNo");
	datT1Z_YesNo.setQueryInfo(new VSQueryInfo("Z_YesNo", "", "", "", false));
	datT1Z_YesNo.setName ("datT1Z_YesNo");
		datT1Z_YesNo.setForeground(Color.black);
		datT1Z_YesNo.setBackground(Color.white);
	datT1Z_YesNo.setBounds (119, 405 , 252, 27);

datT1Z_YesNo.setFont (new Font ("Helvetica", 0, 12));
	datT1Z_YesNo.setVisible(false);

	datT1Z_YesNo.setTag ("_$$_");
 	datT1Z_YesNo.setTabOrder(13);
	datT1Z_YesNo.setTabStop (false);
	setRootDataControl(datT1Z_YesNo);
	datT1Z_YesNo.setMaxRowsPerFetch(16);
	datT1Z_YesNo.setSaveMode (VSJavaDataControl.SAVE_BUFFERED);
	datT1Z_YesNo.setDataDependency (false, false);
        


      // container is form
add (datT1Z_YesNo,null);             
	
	
	} // end of initVSJavaDataControldatT1Z_YesNo()
	...
	public void initVSGridgrdT1Z_YesNo() {
	grdT1Z_YesNo.setName ("grdT1Z_YesNo");
		grdT1Z_YesNo.setForeground(Color.black);
		grdT1Z_YesNo.setBackground(Color.white);
	scrlgrdT1Z_YesNo.setBounds(new Rectangle(4, 37 , 474, 372));

grdT1Z_YesNo.setFont (new Font ("Helvetica", 0, 12));

	grdT1Z_YesNo.setTag ("_$$_");
 	grdT1Z_YesNo.setAutoResizeMode (JTable.AUTO_RESIZE_OFF);
	grdT1Z_YesNo.setColumnHeaderHeight(17);
	grdT1Z_YesNo.setRowSelectionAllowed(false);
	grdT1Z_YesNo.setTabOrder(14);
	grdT1Z_YesNo.setSelectionMode (ListSelectionModel.SINGLE_SELECTION);
	grdT1Z_YesNo.setShowStarRow(true);
	grdT1Z_YesNo.setShowHorizontalLines(true);
	grdT1Z_YesNo.setShowVerticalLines(true);
	grdT1Z_YesNo.setRowHeaderWidth (25);
	grdT1Z_YesNo.setRowHeight (20);
	grdT1Z_YesNo.setSelectionBackground(new Color(0x000080));
	grdT1Z_YesNo.setDisableBackground(new Color(0xffffff));
	grdT1Z_YesNo.setDataSourceName("datT1Z_YesNo");
        grdT1Z_YesNo.setColumnHeaderFont(new Font ("Helvetica", 0, 12));
        scrlgrdT1Z_YesNo.setRowHeaderView(grdT1Z_YesNo.addRowHeaderRenderer());
	
      // container is form
add(scrlgrdT1Z_YesNo,null);         
	try
	{
			//{{GRIDCOLUMN_CTOR
			VSGridColumn gridCol = null;
				gridCol = grdT1Z_YesNo.addColumn ();
				gridCol.setDataFieldName ("Stored");
				gridCol.setLabel ("Stored");
				gridCol.setPreferredWidth (75);
				gridCol.setSortAllowed(true);
				
				gridCol = grdT1Z_YesNo.addColumn ();
				gridCol.setDataFieldName ("Displayed");
				gridCol.setLabel ("Displayed");
				gridCol.setPreferredWidth (75);
				gridCol.setSortAllowed(true);
